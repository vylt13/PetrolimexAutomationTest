...
public class FormTaskVanBanDen {
	WebDriver driver;
...
public FormTaskVanBanDen(WebDriver driver) {
	this.driver = driver;
}
// Button
...
By cBoxPhanCongNguoiXuLy2 = By.id("chbHandle110_723");
...
public void ValidateLinkHoSoXuLy() {
	...
	assertEquals(linkHSXL.contains("HSXL-VBD"), true);
}
...
public void CapNhatNhiemVuDaPhanCong(
		String nguoi_xu_ly_2,
		String nguoi_xu_ly_1) throws InterruptedException {
	driver.findElement(txtlinkNhiemVuDaPhanCong).click();
	WebDriverWait wait = new WebDriverWait(driver, 90);
	wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(4));
	Thread.sleep(2000);
	...
	driver.findElement(By.xpath("//div[contains(@id,'pkNguoiXuLy')]//input[2]")).sendKeys(nguoi_xu_ly_1); /* Nhap user phan cong lai*/
	Thread.sleep(2000);
