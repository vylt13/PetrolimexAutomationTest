...
public class TC11_ChuyenXuLyTaiBuocLanhDaoPhongBan extends ThuocTinhVBDuThao {
	WebDriver driver;
// Format Simple Date
	String trich_yeu = "[VTTest] TC 11 - Chuyển xử lý tại bước Lãnh đạo Phòng ban " + System.currentTimeMillis();
@Test
public void ChuyenXuLyTaiBuocLanhDaoPhongBan() throws InterruptedException {
	...
	login.LogReport();
	formVanBanDuThao.ChuyenXuLy(user_chuyen_xu_ly_ky_so, y_kien_phe_duyet_vb_du_thao);
	login.NavigateToTrangChuVB();
	danhSachViecCanXuLy.ValidateHoanTatTaskPheDuyetVbDuThao(trich_yeu);
	login.signOut();
	login.LoginUserChuTich();
	danhSachViecCanXuLy.ValidateTaskChuyenXuLyPheDuyetVbDuThaoKySo(trich_yeu, lanh_dao_phong_ban);
	danhSachViecCanXuLy.ClickOnFirstItem();
	...
	danhSachViecCanXuLy.ValidateTaskPheDuyetVbDuThaoKySo(trich_yeu, user_chuyen_xu_ly_ky_so);
	danhSachViecCanXuLy.ClickOnFirstItem();
	login.LogReport();
	formVanBanDuThao.DongY(y_kien_phe_duyet_vb_du_thao);
	login.NavigateToTrangChuVB();
