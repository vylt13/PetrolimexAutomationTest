...
public class TC01_NV_NVCapNhatTask extends ThuocTinhVBDen {
	WebDriver driver;
	String trich_yeu = "TC 01 - Nhân viên - Kiểm tra Nhân viên cập nhật Task: " + createdTime;
...
@Test
public void NVCapNhatTask() throws InterruptedException {
	...
	formXuLyVanBanDen.ChuyenXuLyLanhDaoKhac(lanh_dao_tgd);
	formXuLyVanBanDen.waitingForLoading();
	login.NavigatToVBChoYKienLanhDao();
	danhSachVBChoYkien.ValidateVBChoYKienCoDinhKem(
			trich_yeu, 
	...
	danhSachViecCanXuLy.ClickOnFirstItem();
	formTaskVanBanDen.waitingForLoading();
	formTaskVanBanDen.CapNhatTinhTrangDangThucHien(
			y_kien_nv_thuc_hien, 
			tien_do);
	formTaskVanBanDen.Luu();
	danhSachViecCanXuLy.ClickOnFirstItem();
	formTaskVanBanDen.MoPhieuGiaiQuyet();
	formXuLyVanBanDen.ValidateTinhTrangNhanVienDangThucHien(nguoi_xu_ly_1);
	login.LogReport();
	driver.quit();
	} 
}