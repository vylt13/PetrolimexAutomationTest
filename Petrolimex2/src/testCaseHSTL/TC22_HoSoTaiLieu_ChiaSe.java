...
public class TC22_HoSoTaiLieu_ChiaSe extends ThuocTinhHoSoTaiLieu{
	WebDriver driver;
	String noi_dung_hop_dong = "[VTTest] Hồ sơ tài liệu - Chức năng chia sẻ " + createdTime;
@Test
public void HoSoTaiLieu_ChiaSe() throws InterruptedException {
	...
	formHoSoTaiLieu.ChiaSeTrenFormXuLy(user_phe_duyet_hstl, y_kien_phe_duyet_hstl);
	login.signOut();
	login.NavigateToSite();
	login.LoginUserPheDuyetHSTL();
	danhSachViecCanXuLy.ValidateHoanTatNhanChiaSeVBDuThao(noi_dung_hop_dong);
	danhSachViecCanXuLy.ClickOnFirstItem();
	formHoSoTaiLieu.ValidatePermissionView();
	driver.quit();
	} 
}